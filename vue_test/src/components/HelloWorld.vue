<template>
  <div class="hello">
    ...
    <el-table
      ...
      <el-table-column prop="hiredate" label="入职日期"  width="120" text-align="center" >
         <template slot-scope="scope">
        {{formateDate(scope.row.hiredate)}}
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
        ...
        // 要请求的地址
        axios.get("api/pageList",{params:{pageNum:pageNum,pageSize:pageSize,depid:this.formInline.depid, address:this.formInline.address,
          min_bsaralay:this.formInline.min_bsaralay,max_bsaralay:this.formInline.max_bsaralay}}).then(res=>{
          // 把列表信息赋值给tableData
